...
print(x_datasets.shape)   #(2, 100)  .transpose()한 후에 (100, 2)   
...
x_train, x_test, \
    ...
    x_datasets, y, y2, train_size=0.7, random_state=1234
)
...
#2-3 모델병합
from tensorflow.keras.layers import concatenate   # Concatenate               # Concatenate 으로 바꾸기 
...
last_output= Dense(1, name='last_output')(output1)      #선생님이 알려주신 방법 
# main_output = Dense(1, name='main_output')(merge4)     
# last_output = Dense(1, name='last_output')(merge3)    


#2-5 모델5 분기1 #선생님방법
dense5 = Dense(11, activation='linear')(last_output)
dense5 = Dense(22, activation='linear')(dense5)
output5 = Dense(33, activation='linear')(dense5)


#2-5 모델6 분기2 선생님방법
dense6 = Dense(11, activation='linear')(last_output)
dense6 = Dense(22, activation='linear')(dense6)
output6 = Dense(33, activation='linear')(dense6)

# model = Model(inputs=[input1, input2, input3], outputs=[main_output, last_output])  
model = Model(inputs=[input1,], outputs=[output5, output6])   #선생님 방법 
model.summary()

#3. 컴파일 훈련 
model.compile(loss ='mse', optimizer='adam')
model.fit(x_train,[y_train, y2_train], epochs=50, batch_size=8)

#4. 평가 예측
loss = model.evaluate(x_test, [y_test, y2_test])
print('loss : ', loss)                
