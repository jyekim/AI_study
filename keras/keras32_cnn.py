...
model.add(Conv2D(5,(2,2)))       #(N, 3, 3, 5) # dense모델을 할때 히든레이어는 사람이 정하듯 output필터 역시 정하면됨  배치사이즈는 훈련의 단위 
...
model.add(Dense(4,activation= 'relu'))  #지현, 성한, 건률,                      #(input_shape) cov2d 레이어에 입력되는 이미지 크키                #(N, 1)
